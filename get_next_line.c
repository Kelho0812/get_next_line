...
/*   Updated: 2023/10/26 18:26:10 by jorteixe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	buffer_str = ft_calloc(BUFFER_SIZE + 1, 1);
	while (!has_newline(str) && n > 0)
	{
		...
		buffer_str[n] = '\0';
		str = ft_strjoin(str, buffer_str);
	}
	stringzz = ft_substr(str);
...
char	*ft_clean_str(char *str)
{
	...
	while (str[i] && str[i] != '\n')
		i++;
	s1 = ft_calloc(ft_strlen(str) - i + 1, 1);
	while (str[i])
		s1[j++] = str[++i];
	s1[j] = '\0';
	free(str);
